...
!!! tip "AI IDE推荐"
    ...
    推荐使用：VSCode+Cline(需要自己的API Key), Trae, Cursor(订阅付费)
...
这四部分中后三部分不是必须的，请同学们根据实际使用情况自行判断。
...
!!! warning "makefile中一定需要使用Tab键作为开头而非4个空格，因此vscode需要正确设置，以免插件自动补全出错。"
...
1. 在开启 trap 处理处展示 `stvec` 的值，并在 CSR 初始化后查看 `sie` 与 `sstatus` 的值是否被正确设置。

2. 分别展示上下文切换前后各个寄存器的值（上下文）是否被正确 store 和 load，需要展示寄存器的值以及栈中对应地址保存的值。

3. 在 ecall 前后展示受影响的寄存器的值的变化。

4. 尝试在内联汇编部分添加/不添加（与你的实现相反）第四部分，重复3，展示区别。
